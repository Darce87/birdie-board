...
as $$
declare
  event_id uuid; entry jsonb; v_player_id uuid; v_hole integer; v_gross integer;
begin
  select tournament_id into event_id from public.tournament_rounds where id=target_round;
  ...
  for entry in select value from jsonb_array_elements(entries) loop
    ...
    if v_hole not between 1 and 18 or v_gross not between 1 and 20 then raise exception 'Invalid score'; end if;
    if not public.can_manage_tournament(event_id) and not exists (
      ...
      where g.round_id=target_round and g.scorer_id=auth.uid() and gm.user_id=v_player_id
    ...
    values(target_round,v_player_id,v_hole,v_gross,auth.uid())
    on conflict on constraint live_round_hole_scores_pkey do update set gross_score=excluded.gross_score,updated_by=excluded.updated_by,updated_at=now();
  end loop;
end;
$$;
